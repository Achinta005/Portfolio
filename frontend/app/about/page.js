...
export default async function AboutPage() {
  ...
  try {
    const [skillsRes, eduRes, certRes] = await Promise.all([
      fetch(`${process.env.NEXT_PUBLIC_SERVER_API_URL}/about/Skilldata`, {
        next: { revalidate: 3600 },
      }),
      fetch(`${process.env.NEXT_PUBLIC_SERVER_API_URL}/about/Educationdata`, {
        next: { revalidate: 3600 },
      }),
      fetch(
        `${process.env.NEXT_PUBLIC_SERVER_API_URL}/about/Certificatesdata`,
        {
          next: { revalidate: 3600 },
        }
      ),
