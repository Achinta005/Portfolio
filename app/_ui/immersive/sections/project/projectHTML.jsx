...
import { subscribeToScroll } from "../../components/scrollState";
...
function dedupeAndPick(rawList) {
  const seen = new Map();
  for (const p of rawList) { if (!seen.has(p._id)) seen.set(p._id, p); }
...
// ─── MOBILE IMAGE CAROUSEL (shown inline in the mobile card) ─────────────────
function MobileImageCarousel({ project }) {
  ...
  if (!project || project.media.length === 0) {
    ...
    return (
      <div style={{
        ...
        borderRadius: "12px",
        background: `linear-gradient(135deg, rgba(0,4,12,0.9), ${project?.accent ?? "#00b8ff"}10)`,
        border: `1px solid ${project?.accent ?? "#00b8ff"}25`,
        display: "flex", alignItems: "center", justifyContent: "center",
        color: `${project?.accent ?? "#00b8ff"}25`,
        fontSize: "36px",
        flexShrink: 0,
      }}>⬡</div>
    );
  ...
  return (
    <div style={{ position: "relative", width: "100%", flexShrink: 0 }}>
      ...
      <div style={{
        ...
        {current && (
          <img
            ...
            style={{
              ...
              objectFit: "cover",
              display: "block",
              animation: "mediaFadeIn 0.4s ease both",
            }}
        ...
        {/* Top shimmer */}
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "1px", background: `linear-gradient(90deg, transparent, ${project.accent}80, transparent)` }} />
        ...
        <div style={{
          ...
          <span style={{ fontFamily: "monospace", fontSize: "7px", color: `${project.accent}cc`, letterSpacing: "2px", textTransform: "uppercase" }}>Preview</span>
        </div>
        ...
        <div style={{
          ...
          color: `${project.accent}20`, lineHeight: 1, pointerEvents: "none",
        }}>{project.num}</div>

        {/* Bottom gradient fade */}
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "50%", background: "linear-gradient(transparent, rgba(0,4,14,0.95))", pointerEvents: "none" }} />
      </div>
      ...
      {project.media.length > 1 && (
        <div style={{ display: "flex", justifyContent: "center", gap: "5px", marginTop: "8px" }}>
          {project.media.map((_, i) => (
            <div
              key={i}
              onClick={() => { setMediaIdx(i); setMediaFade(f => f + 1); }}
              style={{
                width: i === mediaIdx ? "18px" : "5px",
                height: "5px",
                borderRadius: "100px",
                background: i === mediaIdx ? project.accent : `${project.accent}35`,
                boxShadow: i === mediaIdx ? `0 0 6px ${project.accent}` : "none",
...
function LeftPanel({ project, visible, sectionOpacity, onHoverChange }) {
  ...
  return (
    ...
    >
      ...
      {/* Image area */}
      <div
        style={{ position: "relative", flex: "0 0 62%", overflow: "hidden", background: "rgba(0,4,12,0.9)" }}
      ...
      >
        ...
        <div style={{ position: "absolute", bottom: "10px", left: "14px", right: "14px", pointerEvents: "none" }}>
          <div style={{ marginBottom: "6px" }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", padding: "2px 10px", borderRadius: "100px", background: `${project.categoryColor}22`, border: `1px solid ${project.categoryColor}55`, fontSize: "8px", fontFamily: "monospace", color: project.categoryColor, letterSpacing: "1px", boxShadow: `0 0 12px ${project.categoryColor}22` }}>
          ...
          <div style={{ borderLeft: `3px solid ${project.accent}`, paddingLeft: "9px" }}>
            <h3 style={{ margin: 0, fontSize: "12px", fontWeight: 700, fontFamily: "'JetBrains Mono', 'Fira Code', monospace", color: "rgba(230,245,255,0.96)", lineHeight: 1.4, textShadow: "0 1px 8px rgba(0,0,0,0.8)" }}># {project.title}</h3>
          </div>
        </div>
        {project.media.length > 1 && (
        ...
        {project.links.map((link) => (
          <button
            ...
            style={{
              ...
              background: link.isLive ? `linear-gradient(135deg, ${project.accent}28 0%, ${project.accent}14 100%)` : "rgba(255,255,255,0.04)",
              ...
              width: "100%", transition: "all 0.22s ease", letterSpacing: "0.5px",
              boxShadow: link.isLive ? `inset 0 1px 0 ${project.accent}20` : "none",
            }}
            onMouseEnter={e => {
              e.currentTarget.style.transform = "translateY(-1px)";
              if (link.isLive) { e.currentTarget.style.boxShadow = `0 4px 20px ${project.accent}40, inset 0 1px 0 ${project.accent}30`; e.currentTarget.style.background = `linear-gradient(135deg, ${project.accent}38 0%, ${project.accent}20 100%)`; }
            ...
            onMouseLeave={e => {
              ...
              e.currentTarget.style.background = link.isLive ? `linear-gradient(135deg, ${project.accent}28 0%, ${project.accent}14 100%)` : "rgba(255,255,255,0.04)";
            }}
          >
...
// ─── DESKTOP PROJECT CARD (right panel — text only) ───────────────────────────
function DesktopProjectCard({ project, fadeKey, onHoverChange }) {
  return (
    <motion.div
      key={fadeKey}
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden", minHeight: 0 }}>
        ...
        {/* Title block */}
        <div style={{ flexShrink: 0, marginBottom: "12px" }} onMouseEnter={() => onHoverChange(true)} onMouseLeave={() => onHoverChange(false)}>
          <div style={{ marginBottom: "8px" }}>
            <span style={{
              display: "inline-flex", alignItems: "center", gap: "6px",
              padding: "3px 10px", borderRadius: "100px",
              background: `${project.categoryColor}18`, border: `1px solid ${project.categoryColor}50`,
              fontSize: "10px", fontFamily: "'JetBrains Mono', monospace",
              color: project.categoryColor, letterSpacing: "1.5px", textTransform: "uppercase",
          ...
          <div style={{ borderLeft: `3.5px solid ${project.accent}`, paddingLeft: "14px", position: "relative" }}>
            ...
            <h3 style={{ margin: 0, fontSize: "18px", fontWeight: 700, fontFamily: "'JetBrains Mono', 'Fira Code', monospace", color: "rgba(230,245,255,0.96)", lineHeight: 1.35, letterSpacing: "-0.3px" }}>
              <span style={{ color: `${project.accent}80`, fontSize: "15px" }}># </span>{project.title}
            </h3>
          </div>
        ...
        {/* About */}
        <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none", minHeight: 0, marginBottom: "12px" }} onMouseEnter={() => onHoverChange(true)} onMouseLeave={() => onHoverChange(false)}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" }}>
          ...
          <p style={{ margin: 0, fontSize: "12.5px", lineHeight: 1.65, color: "rgba(185,215,240,0.70)", fontFamily: "'JetBrains Mono', 'Fira Code', monospace", letterSpacing: "0.1px" }}>
            {project.desc}
          </p>
        </div>
        ...
        {project.modelAccuracy != null && (
          <div style={{ flexShrink: 0, paddingTop: "10px", borderTop: `1px solid ${project.accent}15` }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: "8px", padding: "5px 14px", borderRadius: "10px", background: "linear-gradient(135deg, rgba(52,211,153,0.12) 0%, rgba(52,211,153,0.06) 100%)", border: "1px solid rgba(52,211,153,0.35)", fontSize: "12px", fontFamily: "'JetBrains Mono', monospace", color: "#34d399", boxShadow: "0 0 20px rgba(52,211,153,0.12), inset 0 1px 0 rgba(52,211,153,0.15)", letterSpacing: "0.3px" }}>
...
// ─── MOBILE PROJECT CARD (full self-contained card with image) ────────────────
function MobileProjectCard({ project, fadeKey }) {
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        style={{ display: "flex", flexDirection: "column", gap: "10px", flex: 1, overflow: "hidden", minHeight: 0 }}
      >
        {/* ── IMAGE CAROUSEL (always shown on mobile) */}
        <MobileImageCarousel project={project} />
        ...
        <div style={{ flexShrink: 0 }}>
          <span style={{
            ...
            color: project.categoryColor, letterSpacing: "0.8px",
          }}>
            <span style={{ width: "5px", height: "5px", borderRadius: "50%", background: project.categoryColor, boxShadow: `0 0 5px ${project.categoryColor}` }} />
            {project.category}
          </span>
        </div>

        {/* ── Title */}
        <div style={{ flexShrink: 0, borderLeft: `2.5px solid ${project.accent}`, paddingLeft: "10px", position: "relative" }}>
          <div style={{ position: "absolute", left: "-1px", top: 0, bottom: 0, width: "2.5px", background: project.accent, boxShadow: `0 0 8px ${project.accent}`, borderRadius: "2px", pointerEvents: "none" }} />
          <h3 style={{ margin: 0, fontSize: "14px", fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: "rgba(225,242,255,0.96)", lineHeight: 1.35 }}>
            <span style={{ color: `${project.accent}70`, fontSize: "12px" }}># </span>{project.title}
          </h3>
        </div>

        {/* ── Tech tags — horizontal scroll */}
        <div style={{ flexShrink: 0, overflowX: "auto", WebkitOverflowScrolling: "touch", scrollbarWidth: "none" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none", minHeight: 0 }}>
          <p style={{ margin: 0, fontSize: "11px", lineHeight: 1.65, color: "rgba(180,210,235,0.72)", fontFamily: "'JetBrains Mono', monospace" }}>
            {project.desc}
          </p>
        </div>
          ...
          {project.links.map(link => (
            <motion.button
              ...
              style={{
                ...
                background: link.isLive ? `${project.accent}20` : "rgba(255,255,255,0.05)",
                border: `1px solid ${link.isLive ? project.accent + "60" : "rgba(255,255,255,0.12)"}`,
                fontSize: "10px", fontFamily: "monospace",
                color: link.isLive ? project.accent : "rgba(200,220,240,0.6)",
                cursor: link.href ? "pointer" : "default",
                boxShadow: link.isLive ? `0 0 12px ${project.accent}20` : "none",
              }}
            >
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        color: "rgba(0,210,255,0.8)", borderRadius: "50%",
        width: "34px", height: "34px",
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: "15px", cursor: "pointer", fontFamily: "monospace",
...
function useSwipe(onSwipeLeft, onSwipeRight) {
  ...
  const onTouchEnd = useCallback((e) => {
    ...
    if (Math.abs(dx) > 45) { dx < 0 ? onSwipeLeft() : onSwipeRight(); }
    startX.current = null;
...
export default function ProjectsHTML() {
  ...
  // Mobile card height: tall enough for image + content
  const mobileCardHeight = "min(88vh, 680px)";
  ...
  return (
    <>
      ...
      {/* Desktop left panel */}
      {hydrated && !isMobile && !loading && !error && project && (
        <LeftPanel
          project={project}
      ...
      <div
        ...
        style={{
          ...
          // Responsive sizing
          width: isMobile ? "92vw" : DESKTOP_CARD_WIDTH,
          maxWidth: isMobile ? "440px" : undefined,
          height: isMobile ? mobileCardHeight : DESKTOP_CARD_HEIGHT,
          ...
          background: "linear-gradient(145deg, rgba(0,5,15,0.90) 0%, rgba(0,8,22,0.84) 100%)",
          ...
          // Touch pan support for scroll inside card
          touchAction: "pan-y",
        }}
      >
        {/* Top shimmer */}
        <div style={{ position: "absolute", top: 0, left: "10%", right: "10%", height: "1px", background: "linear-gradient(90deg, transparent, rgba(0,210,255,0.7), transparent)", opacity: 0.5, flexShrink: 0 }} />

        {/* Header */}
        <div style={{ marginBottom: isMobile ? "10px" : "12px", textAlign: "center", flexShrink: 0 }}>
          <h2 style={{
        ...
        {!loading && !error && project && (
          <>
            {/* Render correct card variant */}
            {isMobile ? (
              <MobileProjectCard project={project} fadeKey={fadeKey} />
            ) : (
              <DesktopProjectCard project={project} fadeKey={fadeKey} onHoverChange={handleHoverChange} />
            )}

            {/* Navigation */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: isMobile ? "10px" : "14px", marginTop: isMobile ? "8px" : "10px", flexShrink: 0 }}>
              <NavBtn onClick={goPrev} label="Previous">‹</NavBtn>

              {/* Dot indicators — limit to 8 on mobile to avoid overflow */}
              <div style={{ display: "flex", gap: "5px", alignItems: "center", overflow: "hidden", maxWidth: isMobile ? "160px" : "unset" }}>
                {projects.slice(0, isMobile ? 8 : projects.length).map((_, i) => (
                  <motion.div
                    key={i}
                    onClick={() => goTo(i)}
                    animate={{
                    ...
                    style={{ width: "6px", height: "6px", borderRadius: "50%", border: "1px solid rgba(0,210,255,0.3)", cursor: "pointer", flexShrink: 0 }}
                  />
                ))}
                {isMobile && projects.length > 8 && (
                  <span style={{ color: "rgba(0,210,255,0.3)", fontFamily: "monospace", fontSize: "9px" }}>…</span>
                )}
              </div>
            ...
            <div style={{ textAlign: "center", marginTop: "4px", flexShrink: 0 }}>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: isMobile ? "10px" : "11px", color: "rgba(0,210,255,0.35)", letterSpacing: "3px" }}>
                {String(currentIndex + 1).padStart(2, "0")} <span style={{ color: "rgba(0,210,255,0.18)" }}>/</span> {String(projects.length).padStart(2, "0")}
              </div>
              {isMobile && (
                <div style={{ fontFamily: "monospace", fontSize: "8px", color: "rgba(0,210,255,0.20)", letterSpacing: "1.5px", marginTop: "2px" }}>
                  ← swipe to navigate →
                </div>
              )}
            </div>
        ...
        {/* Bottom shimmer */}
        <div style={{ position: "absolute", bottom: 0, left: "10%", right: "10%", height: "1px", background: "linear-gradient(90deg, transparent, rgba(0,210,255,0.4), transparent)", flexShrink: 0 }} />
      </div>
    </>
  );
}